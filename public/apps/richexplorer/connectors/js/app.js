...
const config = require('../../config/filemanager.config.json');
paths.posix = require('path-posix');
var __appRoot = 'C:/xampp/htdocs/RichFilemanager/userfiles';
const upload = multer({dest: 'public/'});
	...
	app.get('/', function(req, res){ // eslint-disable-line complexity
		...
		switch (mode.trim()) {
			case 'initiate':
				respond(res, JSON.parse('{"data":{"id":"\/","type":"initiate","attributes":{"config":{"security":{"readOnly":false,"extensions":{"policy":"ALLOW_LIST","ignoreCase":true}},"upload":{"fileSizeLimit":16000000},"viewer":{"absolutePath":null,"previewUrl":"/userfiles/"}}}}}'));
	...
	app.post('/', upload.array('files', config.upload.maxNumberOfFiles), (req, res) => {
...
app.use(express.static('C:/xampp/htdocs/RichFilemanager/'));
