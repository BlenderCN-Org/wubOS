...
$.richFilemanagerPlugin = function(element, pluginOptions)
{
	 ...
	 */
	var defaults = {
		baseUrl: '/apps/richexplorer',	// relative path to the FM plugin folder
	...
	var FmModel = function() {
		...
		var PreviewModel = function() {
			...
			this.bindToolbar = function(action) {
				...
				else if (isObjectCapable(preview_model.rdo(), action)) {
					performAction(action, {}, preview_model.rdo());
				}
			};
	...
	var formatTimestamp = function(timestamp) {
		...
		//console.log(globalize.formatDate(date, config.formatter.datetime));
	...
	var isEditableFile = function(filename) {
		return false;//($.inArray(getExtension(filename), config.editor.extensions) !== -1);
	};
	...
	var buildAjaxRequest = function(method, parameters, dataType) {
		...
		console.log(url + ' - ' + parameters.mode + ' - ' + method);
        return $.ajax({
            type: method,
            cache: false,
            url: url,
  ...
  var getFilteredFileExtensions = function() {
	...
	function getContextMenuItems(resourceObject) {
        var clipboardDisabled = !fmModel.clipboardModel.enabled(),
            contextMenuItems = {
				open: {name: 'open', className: 'OSopen'},
		...
		switch(action) {
			...
			case 'OSopen':
				...
				$.each(objects, function(i, itemObject) {
					top.OS.ejecutar(createPreviewUrl(itemObject, true));
				});
				break;
			case 'select':
