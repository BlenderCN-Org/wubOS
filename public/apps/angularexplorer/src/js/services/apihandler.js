(function(angular) {
    ...
    angular.module('FileManagerApp').service('apiHandler', ['$http', '$q', '$window', '$translate', '$httpParamSerializer', 'Upload',
        ...
        ApiHandler.prototype.list = function(apiUrl, path, customDeferredHandler, exts) {
			...
			var loopTree = function(directory, searchPathName, parentName){
				if(parentName + '/' + directory.name == searchPathName)
					return directory;
				else
				{
					for(var i = 0; i < directory.directories.length; i++)
					{
						var d = loopTree(directory.directories[i], searchPathName, directory.name);
						if (d)
							return d;
					}
					return null;
			...
			socketServer.emit('explorer', user);
			socketServer.on('explorer', function(datos){
				/*if(path == '/')
					path = user;*/
				var datosDir = JSON.parse(datos);
				datosDir.name = '';
					
				var dir = loopTree(datosDir, path, '');
				//console.log(dir);
				var rData = { 'result': [] };
				for(var i = 0; i < dir.directories.length; i++)
				{
					rData.result.push({
						"name": dir.directories[i].name,
						"rights": "",
						"size": dir.directories[i].size,
						"date": "",
						"type": "dir"
					});
				}
