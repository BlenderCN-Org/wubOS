...
var wubOS = function(numEsc)
{
	...
	var loadEscritorios = function(){
		...
		this.baseEscritorios.style.width = "100%";
		...
		this.baseEscritorios.appendChild(wrapper);
		...
		for(var i = 0; i< this.numeroEscritorios; i++)
		{
			...
			this.escritorios[i].className = 'swiper-slide escritorio';
			this.escritorios[i].style.width = "100%";
			this.escritorios[i].style.backgroundColor = getRandomColor();
			//this.escritorios[i].style.left = (i * window.innerWidth) + "px";
			this.escritorios[i].pos = i;
			wrapper.appendChild(this.escritorios[i]);
			escItems['cfe' + i] = {"name": "Escritorio " + i, "icon": "fa-picture-o"};
		}
		document.body.appendChild(this.baseEscritorios);
		
		swiper = new Swiper('#baseEscritorios', {
			effect: 'cube',
			grabCursor: true,
			cubeEffect: {
				shadow: true,
